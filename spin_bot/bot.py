...
class OmniMachineV31Refined:
    ...
    async def run_accuracy_cycle(self):
        """V5.23.1: OMNI-RECURSIVE AURORA MACHINE."""
        print(f"--- OMNI MACHINE CYCLE V5.23.1 (AURORA PROTOCOL) ---")
        ...
        try:
            ...
            print("DEBUG: Starting V5.23.1 Aurora UI Protocol...")
            ...
            # V5.23.1: Sequence-Hash Deduplication
            ...
            if spin_count < 200:
                ...
                # V5.23.1: Markov Confidence Escalation
                ...
                if mm_conf < 0.60:
                    print(f"V5.23.1 TUITION: Markov Confidence {mm_conf:.2f} < 60%. Staying in Observation.")
                    return

                self.session_state["mode"] = "SNIPER"
                print(f"V5.23.1 PROMOTION: SNIPER Activated. (Markov Conf: {mm_conf:.2f} | Win Rate: {win_rate:.2f})")
            else:
            ...
            print(f"CRITICAL ERROR in V5.23.1 Cycle: {e}")
            try: await client.capture_failure_artifact("cycle_crash")
            except: pass
        finally:
            # V5.23.1: Mandatory Persistence Commit (Self-Healing)
            ...
            await client.close()
            self.memory.close()
            print(f"--- V5.23.1 CYCLE COMPLETE (Bankroll: ₦{self.risk.bankroll:.2f}) ---")
