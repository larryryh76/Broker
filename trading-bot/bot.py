...
class FoundationBot:
    ...
    def run_snapshot_cycle(self):
        ...
        try:
            ...
            # Save account snapshot to text file for auditing as per memory
            snap_text_path = os.path.join(config.LOG_DIR, "account_snapshot.txt")
            with open(snap_text_path, "a") as f:
                f.write(f"[{snapshot['timestamp']}] Balance: ${snapshot['balance']} | Equity: ${snapshot['equity']}\n")
